...
import { useAreasIncidencia } from '../../../hooks/useTickets'

export function GraficoAreasIncidencia() {
  const [filtros, setFiltros] = useState<FiltrosPeriodo>({ ano: '', mes: '', localidade: '' })
  const { data, isLoading } = useAreasIncidencia(filtros)
  ...
  return (
    <div className="relative bg-white border-2 border-[#e0e0e0] rounded-[5px] h-full flex flex-col">
      ...
      <div className="px-4 pt-[50px] pb-2 border-b border-[#e0e0e0] mx-4">
        <h3 className="text-[18px] font-bold text-[#1d5358]">Áreas com maior incidência de tickets</h3>
