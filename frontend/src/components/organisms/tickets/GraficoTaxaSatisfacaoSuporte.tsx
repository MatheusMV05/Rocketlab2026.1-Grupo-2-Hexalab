...
import { useTaxaSatisfacaoSuporte } from '../../../hooks/useTickets'
...
export function GraficoTaxaSatisfacaoSuporte() {
  const [filtros, setFiltros] = useState<FiltrosPeriodo>({ ano: '', mes: '', localidade: '' })
  const { data, isLoading } = useTaxaSatisfacaoSuporte(filtros)

  const VALOR = data?.valor ?? 62
    ...
    <div className="relative bg-white border-2 border-[#e0e0e0] rounded-[5px] h-full flex flex-col">
      ...
      <div className="px-4 pt-[50px] pb-1">
        <h3 className="text-[18px] font-bold text-[#1d5358] leading-snug">
          Taxa de satisfação (suporte)
        </h3>
      </div>

      <div className="flex-1 flex flex-col items-center justify-center min-h-0 px-4 pb-4">
        <svg viewBox="0 0 560 215" width="100%" style={{ display: 'block', overflow: 'visible' }} aria-hidden="true">
          <defs>
            <filter id="meta-shadow-suporte" x="-20%" y="-40%" width="160%" height="200%">
          ...
          <g filter="url(#meta-shadow-suporte)">
            <path
              d={`
        ...
        <div className="flex flex-col items-center -mt-2">
          ...
          <span className="text-[12px] font-medium text-[#4d4d4d] mt-2">
            {isLoading ? '' : `Baseado em ${TOTAL} tickets`}
          </span>
        </div>
      </div>
    </div>
